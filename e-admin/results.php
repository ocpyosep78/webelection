...
//Purpose: Function to display the results of the election
...
while($row = mysql_fetch_array($sqlresult)) {
	...
	// for each position loop through each candidate.
	...
	while($row2 = mysql_fetch_array($sql2result)) {
		...
		$sql3 = "SELECT * FROM votes WHERE votes.can = '".$row2['rowid']."'";
	...
	$sql2 = "SELECT * FROM votes WHERE votes.can = 'noConfidence' AND votes.pos = '".mysql_real_escape_string($row['position'])."'";
	$sql2result = mysql_query($sql2);
	if (!$sqlresult) {
 	   die('Invalid query: ' . mysql_error());
	}
	...
	$sql2 = "SELECT * FROM votes WHERE votes.can = 'noVote' AND votes.pos = '".mysql_real_escape_string($row['position'])."'";
	$sql2result = mysql_query($sql2);
	if (!$sql2result) {
 	   die('Invalid query: ' . mysql_error());
	}
